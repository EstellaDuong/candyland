...
Player HiddenTreasures::staminaRefill(Player player)
{
    ...
    cout<<ORANGETEXT<<"You got a Stamina Refill of "<< added_stamina <<". You now have "<<player.getStamina()<<" stamina."<<RESET<<endl;
    return player;
}
...
Player HiddenTreasures::goldWindfall(Player player)
{
    ...
    cout<<ORANGETEXT<<"You got a Gold Windfall of "<< added_gold <<". You now have "<<player.getGold()<<" gold."<<RESET<<endl;
    return player;
}
...
Player HiddenTreasures::robbersRepel(Player player)
{
    ...
    cout<<ORANGETEXT<<"You got Robber's Repel, a powerful anti-robbery shield."<<RESET<<endl;
    return player; 
}
...
Player HiddenTreasures::candyAquisition(Player player)
{
   ...
   if (chance>=1 && chance<=7)
    ...
    cout<<ORANGETEXT<<"You got Jellybean of Vigor, which regenerates 50 stamina."<<RESET<<endl;
    }
   else
    {
    added_candy = {"Treasure Hunter's Truffle", "This candy allows the player to unlock a hidden treasure", "other", 0, "N/A", 0};
    cout<<ORANGETEXT<<"You got Treasure Hunter's Truffle, which unlocks another Hidden Treasure."<<RESET<<endl;
    }
...
Player HiddenTreasures::runHiddenTreasure(Player player)
{
    ...
    if(is_on_HT_tile)
    {
        cout<<ORANGETEXT<<"You've discovered a Hidden Treasure! Solve this riddle to open it:"<<endl;
...
bool HiddenTreasures::solveRiddle()
{
    ...
    cout<<ORANGETEXT<<"Your answer: "<<RESET<<endl;
    cin.clear();
    cin.ignore(999,'\n');
    ...
    {
        cout<<ORANGETEXT<<"Correct!! Opening Hidden Treasure..."<<RESET<<endl;
        return true;
    }
    else
    {
        cout<<REDTEXT<<"Wrong!!"<<RESET<<endl;
        return false;
    }
}